...
export default function TodoList() {
  ...
  const fetchTasks = async () => {
    try {
      const response = await fetch(`${API_URL}/user/yara`);
      if (!response.ok) {
  ...
  const addTask = async () => {
    if (newTask.trim() !== '') {
      const newTaskObj = { label: newTask, done: false };
  ...
  const clearAllTasks = async () => {
    const updatedTasks = [{ label: "sample task", done: false }];
    setTasks(updatedTasks);
    await updateTasksOnServer(updatedTasks);
  };
      ...
      <div>
        {tasks.map((task, index) => (
          <div key={index} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
            ...
            <button onClick={() => deleteTask(index)} style={{ backgroundColor: '#ff4d4d', color: 'white', border: 'none', padding: '5px 10px', cursor: 'pointer' }}>Eliminar</button>
          </div>
        ))}
      </div>
      <div style={{ marginTop: '20px' }}>
